...
(defcustom toggle-test-map
  ...
  "A list of (RE . TRANS) rules to switch to/from tests."
  :group 'toggle
  :type '(repeat (cons string string)))

(defcustom toggle-fixture-map
  '(("app/models/\\(.*\\).rb" . "test/fixtures/\\1s.yml")
    ("test/fixtures/\\(.*\\).yml" . "app/models/\\1.rb")
    ("test/unit/\\(.*\\)_test.rb" . "test/fixtures/\\1.yml"))
  "A list of (RE . TRANS) rules to switch to/from fixtures."
  :group 'toggle
  :type '(repeat (cons string string)))
...
match is found, switches to that buffer."
  (interactive)
  (let ((new-name (toggle-filename (buffer-file-name) toggle-test-map)))
    (if new-name
	(find-file new-name)
      (message (concat "Match not found for " (buffer-file-name))))))

(defun toggle-fixture ()
  "Opens a related file to the current buffer using matching rules.
Matches the current buffer against rules in toggle-mappings. If a
match is found, switches to that buffer."
  (interactive)
  (let ((new-name (toggle-filename (buffer-file-name) toggle-fixture-map)))
    (if new-name
	(find-file new-name)
      (message (concat "Match not found for " (buffer-file-name))))))
