...
def current_ip
  router_password = File.read(File.expand_path('~/.router_password')).strip
...
if File.read(File.expand_path('~/.ip')).strip != current_ip
  puts "IP has changed!"
  File.open(File.expand_path('~/.new_ip'), 'w'){ |f| f.puts current_ip }
